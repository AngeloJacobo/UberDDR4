...
`i_controller_clk`, uses a **4-byte word address** from 0 to 15, returns a registered
one-cycle ACK and never asserts STALL when enabled. 
...
| 0xc / 0x30 | CONTROL | [2] automatic BIST-failure recovery enabled; [1] write-1 to request recalibration (always reads as zero); [0] write-1 to request BIST (always reads as zero) |
...
eight bits of each nine-bit delay count. When `BIST_MODE=0`, BIST_STATUS [5:0] and the comparison counters read zero;
init status remains available.
...
reads, inspect per-lane `gate_trained_mcl` / `gate_trained_mcl_low. 
